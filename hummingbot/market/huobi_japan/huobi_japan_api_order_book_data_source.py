...
class HuobiJapanAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_active_exchange_markets(cls) -> pd.DataFrame:
        ...
        async with aiohttp.ClientSession() as client:
            ...
            market_response, exchange_response = await safe_gather(
                client.get(HUOBI_JAPAN_TICKER_URL),
                client.get(HUOBI_JAPAN_SYMBOLS_URL)
            )
            market_response: aiohttp.ClientResponse = market_response
            exchange_response: aiohttp.ClientResponse = exchange_response

            if market_response.status != 200:
                raise IOError(f"Error fetching Huobi Japan markets information. "
                              f"HTTP status is {market_response.status}.")
            if exchange_response.status != 200:
                raise IOError(f"Error fetching Huobi Japan exchange information. "
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str) -> Dict[str, Any]:
        ...
        async with client.get(HUOBI_JAPAN_DEPTH_URL, params=params) as response:
            response: aiohttp.ClientResponse = response
            if response.status != 200:
                raise IOError(f"Error fetching Huobi Japan market snapshot for {trading_pair}. "
        ...
        async with aiohttp.ClientSession() as client:
            ...
            for index, trading_pair in enumerate(trading_pairs):
                try:
                    ...
                    snapshot_msg: OrderBookMessage = HuobiJapanOrderBook.snapshot_message_from_exchange(
                        snapshot,
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        if "ping" in msg:
                            ...
                            for data in msg["tick"]["data"]:
                                trade_message: OrderBookMessage = HuobiJapanOrderBook.trade_message_from_exchange(
                                    data, metadata={"trading_pair": trading_pair}
                                )
                                output.put_nowait(trade_message)
                        else:
                            self.logger().debug(f"Unrecognized message received from Huobi Japan websocket: {msg}")
                ...
                async with websockets.connect(HUOBI_JAPAN_WS_URI) as ws:
                    ...
                    async for raw_msg in self._inner_messages(ws):
                        ...
                        if "ping" in msg:
                            ...
                            order_book_message: OrderBookMessage = HuobiJapanOrderBook.diff_message_from_exchange(msg)
            ...
            try:
                ...
                async with aiohttp.ClientSession() as client:
                    for trading_pair in trading_pairs:
                        try:
                            snapshot: Dict[str, Any] = await self.get_snapshot(client, trading_pair)
                            snapshot_message: OrderBookMessage = HuobiJapanOrderBook.snapshot_message_from_exchange(
                                snapshot,
                                metadata={"trading_pair": trading_pair}
                            )

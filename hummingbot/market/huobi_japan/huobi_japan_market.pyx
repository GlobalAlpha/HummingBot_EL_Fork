...
from hummingbot.market.huobi_japan.huobi_japan_api_order_book_data_source import HuobiJapanAPIOrderBookDataSource
from hummingbot.market.huobi_japan.huobi_japan_auth import HuobiJapanAuth
from hummingbot.market.huobi_japan.huobi_japan_in_flight_order import HuobiJapanInFlightOrder
...
cdef class HuobiJapanMarketTransactionTracker(TransactionTracker):
    cdef:
        HuobiJapanMarket _owner

    def __init__(self, owner: HuobiJapanMarket):
        super().__init__()
        self._owner = owner
...
cdef class HuobiJapanMarket(MarketBase):
    ...
    def __init__(self,
                 huobi_japan_api_key: str,
                 huobi_japan_secret_key: str,
        ...
        self._huobi_japan_auth = HuobiJapanAuth(api_key=huobi_japan_api_key, secret_key=huobi_japan_secret_key)
        self._in_flight_orders = {}
        self._last_poll_timestamp = 0
        self._last_timestamp = 0
        self._order_book_tracker = HuobiJapanOrderBookTracker(
        ...
        self._tx_tracker = HuobiJapanMarketTransactionTracker(self)
    ...
    def convert_from_exchange_trading_pair(exchange_trading_pair: str) -> Optional[str]:
        if HuobiJapanMarket.split_trading_pair(exchange_trading_pair) is None:
            return None
        # Huobi uses lowercase (btcusdt)
        base_asset, quote_asset = HuobiJapanMarket.split_trading_pair(exchange_trading_pair)
    ...
    @property
    def order_book_tracker(self) -> HuobiJapanOrderBookTracker:
    ...
    @property
    def in_flight_orders(self) -> Dict[str, HuobiJapanInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, Any]):
        self._in_flight_orders.update({
            key: HuobiJapanInFlightOrder.from_json(value)
    ...
    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await HuobiJapanAPIOrderBookDataSource.get_active_exchange_markets()
    ...
    async def _api_request(self,
        ...
        url = HUOBI_JAPAN_ROOT_API + path_url
        client = await self._http_client()
        if is_auth_required:
            params = self._huobi_japan_auth.add_auth_to_params(method, path_url, params)
        ...
        async with response_coro as response:
            ...
            if data is None:
                ...
                raise HuobiJapanAPIError({"error": parsed_response})
            return data
    ...
    cdef object c_get_fee(self,
        ...
        if order_type is OrderType.LIMIT and fee_overrides_config_map["huobi_japan_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["huobi_japan_maker_fee"].value)
        if order_type is OrderType.MARKET and fee_overrides_config_map["huobi_japan_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["huobi_japan_taker_fee"].value)
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for tracked_order in tracked_orders:
                ...
                except HuobiJapanAPIError as e:
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Huobi Japan. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Huobi Japan. "
    ...
    async def execute_buy(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting buy {order_type_str} order to Huobi Japan for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price if order_type is OrderType.LIMIT else ''}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit buy order to Huobi Japan. Check API key and network connection."
            )
    ...
    async def execute_sell(self,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting sell {order_type_str} order to Huobi Japan for "
                f"{decimal_amount} {trading_pair} "
                f"{decimal_price if order_type is OrderType.LIMIT else ''}.",
                exc_info=True,
                app_warning_msg=f"Failed to submit sell order to Huobi Japan. Check API key and network connection."
            )
            self.c_trigger_event(self.MARKET_ORDER_FAILURE_EVENT_TAG,
                                 MarketOrderFailureEvent(self._current_timestamp, order_id, order_type))
        ...
        try:
            ...
            if order_state == 7:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg=f"Failed to cancel the order {order_id} on Huobi Japan. "
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel all orders on Huobi Japan. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object price,
                                object amount):
        self._in_flight_orders[client_order_id] = HuobiJapanInFlightOrder(
